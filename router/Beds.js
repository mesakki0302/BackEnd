...
router.post('/add-ward-and-beds', async (req, res) => {
    try {
      ...
      const existingWard = await Bed.findOne({
        'wards.name': wardName,
      ...
      // Create a new ward with the specified name and wardType
      const newWard = {
        name: wardName,
        wardType: wardType,
        beds: [],
      ...
      // Add 10 beds to the newly created ward
      for (let i = 1; i <= 10; i++) {
        const newBed = {
          number: `Bed ${i}`,
          status: 'available',
        };
        newWard.beds.push(newBed);
      }
  
      const ward = new Bed({ wards: [newWard] });
  
      await ward.save();
  
      res.status(201).json({ message: 'Added 10 extra beds to the specified ward successfully' });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Failed to add the ward and beds' });
    }
  });
...
router.put('/transfer/:patientId', async (req, res) => {
  ...
  try {
    ...
    transfer.transferReason = transferReason || transfer.transferReason;
