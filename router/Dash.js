...
router.post('/admit1', async (req, res) => {
  const {
    name,
    age,
    gender,
...
//configuration screen for adding extra bed number:
// Define a POST route to add an extra bed to both wards


router.post('/addextrabed', async (req, res) => {
  const { wardName, bedNumber } = req.body;

  try {
    // Find all wards with the matching name
    const wards = await Bed.find({ 'wards.name': wardName });

    if (!wards || wards.length === 0) {
      return res.status(404).json({ message: 'Ward not found.' });
    }

    let bedAlreadyExists = false;

    // Check if the bed number is unique within all matching wards
    for (const ward of wards) {
      if (ward.wards && ward.wards.some((w) => w.name === wardName && w.beds.some((bed) => bed.number === bedNumber))) {
        bedAlreadyExists = true;
        break;
      }
    }

    if (bedAlreadyExists) {
      return res.status(400).json({ message: 'Bed number already exists in one of the wards.' });
    }

    // Add the extra bed with details to each matching ward
    for (const ward of wards) {
      const matchingWard = ward.wards.find((w) => w.name === wardName);
      if (matchingWard && matchingWard.beds) {
        matchingWard.beds.push({
          number: bedNumber,
          status: 'available',
          // Other fields...
        });
    ...
    res.status(201).json({ message: 'Extra bed added successfully.' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
  }
...
router.post('/addextrabed1', async (req, res) => {
  const { wardName, bedNumber } = req.body;

  try {
    // Find all wards with the matching name (assuming you have a Bed model)
    const wards = await Bed.find({ 'wards.name': wardName });
    ...
    // Check if the bed number is unique within all matching wards
    for (const ward of wards) {
      if (ward.beds && ward.beds.some((bed) => bed.number === bedNumber)) {
        bedAlreadyExists = true;
        break;
      }
    }
...
router.post('/add', async (req, res) => {
  ...
  try {
    // Find the ward by name (assuming you have a Bed model)
    let ward = await Bed.findOne({ 'wards.name': wardName });

    if (!ward) {
      // Ward doesn't exist, so create a new ward
      ward = new Bed({
        wards: [
          {
            name: wardName,
    ...
    // Check if the bed number is unique within the selected ward
    const existingBed = ward.wards[0].beds.find((bed) => bed.number === bedNumber);

    if (existingBed) {
      return res.status(400).json({ message: 'Bed number already exists in the selected ward.' });
    }

    // Add the extra bed with details to the selected ward
    ward.wards[0].beds.push({
      number: bedNumber,
      status: 'available'      // Other fields...
    });

    // Save the updated ward (either newly created or existing)
    await ward.save();

    res.status(201).json({ message: 'Extra ward and bed added successfully.' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
  }
...
// });
// Function to generate a unique patient ID
function generatePatientID(ward, bedNumber) {
  return `${ward}_${bedNumber}`;
}
...
router.post('/admitpt1', async (req, res) => {
  ...
  try {
    ...
    const existingPatient = await Patient.findOne({
      patientId: generatePatientID(ward, bedNumber),
    });
    ...
    const patientId = generatePatientID(ward, bedNumber);
    const riskScore = calculateRiskScore(medicalAcuity);

    // Rest of the code to admit the patient
